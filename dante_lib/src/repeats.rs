...
mod tests {
    ...
    fn repeat_can_be_parsed() {
        ...
        let tr: HGVSNomenclature = s.parse().unwrap();
        println!("{:?}", tr);
    }

    #[test]
    fn test_inner_motif_view() {
        let motif: HGVSNomenclature = "S1:g.1_10A[10]".parse().unwrap();
...
pub(crate) struct HGVSNomenclature {
...
pub(crate) struct ParseTandemRepeatError;

impl FromStr for HGVSNomenclature {
...
impl fmt::Display for HGVSNomenclature {
...
fn fmt_is_inverse_to_parse() {
    ...
    let motif2 = format!("{}", motif1.parse::<HGVSNomenclature>().unwrap());
...
fn tandem_repeat(input: &str) -> IResult<&str, HGVSNomenclature> {
...
impl HGVSNomenclature {
    ...
    #[cfg(test)]
    pub(crate) fn view(&self, from: usize, to: usize) -> Vec<u8> {
